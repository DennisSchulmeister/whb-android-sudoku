...
/**
 * Background operation to perform a database operation on a SavedGame instance. This can either
...
public class SaveOrDeleteGame implements Runnable {
    ...
    public interface Callback {
         ...
         */
        void onErrorsFound(Map<SavedGame.Error, String> errors);
    };

    /**
     * Constructor.
     *
     * @param savedGame The game to save or delete
     */
    public SaveOrDeleteGame(SavedGame savedGame, Operation operation) {
    ...
    public void run() {
        ...
        if (this.operation == Operation.INSERT || this.operation == Operation.UPDATE) {
            Map<SavedGame.Error, String> errors = this.savedGame.checkConsistency();
        ...
        switch (this.operation) {
            case INSERT:
                this.dao.insert(this.savedGame);
                this.operation = Operation.UPDATE;
                break;
            case UPDATE:
                this.dao.update(this.savedGame);
                break;
            case DELETE:
                this.dao.delete(this.savedGame);
                break;
        }
